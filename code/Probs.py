...
def get_tokens(file: Path, vocab: Optional[Set[Wordtype]] = None) -> Iterable[Wordtype]:
    ...
    # Whenever the `for` loop needs another token, get_tokens picks up where it
...
def num_tokens(file: Path, vocab=None) -> int:
    """Give the number of tokens in file, including EOS."""
    return sum(1 for token in get_tokens(file, vocab))


def get_trigrams(file: Path, vocab=None) -> Iterable[Trigram]:
    """Iterator over the trigrams in file.  Each triple (x,y,z) is a token z
    (possibly EOS) with a left context (x,y)."""
    x, y = BOS, BOS
    for z in get_tokens(file, vocab):
        yield (x, y, z)
        if z == EOS:
...
def draw_trigrams_forever(file: Path, vocab=None, randomize=False) -> Iterable[Trigram]:
    ...
    """
    trigrams = get_trigrams(file, vocab)
    if not randomize:
        import itertools
...
def extract_vocab(vocab_path: Path):
    log.info(f"Extracting vocab from {vocab_path}")
    vocab = set()
    with open(vocab_path, "rt") as fin:
        for line in fin:
            word = line.strip().split("\t")[0] # take the word itself ignore embeddings if any
            vocab.add(word)
    log.info(f"Extracted vocab of size {len(vocab)} from {vocab_path}")
    return vocab
...
class LanguageModel:
    ...
    def __init__(self, lexicon: Path):
        ...
        self.vocab: Set[Wordtype] = extract_vocab(lexicon)
    ...
    # A caller can use this constant for documentation of the `smoother` parameter.
    SMOOTHERS = """uniform, add1, backoff_add1, loglin1, improved
    (the "1" in add1/backoff_add1 can be replaced with any real λ ≥ 0;
    the "1" in loglin1 can be replaced with any real C ≥ 0)"""

    @classmethod
    def make(cls, smoother: str, lexicon: Path, vocab: Path) -> LanguageModel:
        """Factory pattern: return a language model of the kind that
        was requested by the `smoother` `vocab` and `lexicon` arguments.
        This will be an instance of some appropriate subclass 
        of LanguageModel."""

        # Parse the `smoother` argument, which is a complicated string.

        r = re.compile("^(.*?)-?([0-9.]*)$")
        m = r.match(smoother)

        param: Optional[float]  # Type annotation only.
        ...
        if m is None or not m.lastindex:
            ...
            if m.lastindex >= 2 and len(m.group(2)):
                param_arg = m.group(2)
                param = float(param_arg)
            else:
                param = None

        if param is None and smoother_name.find("add") != -1:
            raise ValueError(
        ...
        if smoother_name == "uniform":
            return UniformLanguageModel(vocab)
        elif smoother_name == "add":
            assert param is not None    # param should specify lambda
            return AddLambdaLanguageModel(vocab, param)
        elif smoother_name == "backoff_add":
            assert param is not None    # param should specify lambda
            return BackoffAddLambdaLanguageModel(vocab, param)
        elif smoother_name == "loglin":
            assert param is not None    # param should specify C
            return EmbeddingLogLinearLanguageModel(vocab, param, lexicon)
        elif smoother_name == "improved":
            return ImprovedLogLinearLanguageModel(vocab, param, lexicon)
        else:
            raise ValueError(f"Don't recognize smoother name {smoother_name}")

    # This is the main function that will be used by our callers to solve
    # other tasks.  It's appropriate for the language model to provide
    # this function, since the caller shouldn't have to know that we're a
    # trigram model rather than an RNN model or a PCFG model, nor should
    # it have to know how we tokenize the file.

    def file_log_prob(self, file: Path) -> float:
        """Compute the total log probability under this language model 
        of all sequences of tokens in `file`.  This is a natural log, as
        for all our internal computations.  
        """
        log_prob = 0.0
        assert self.vocab is not None
        for (x, y, z) in get_trigrams(file, self.vocab):
            prob = self.prob(x, y, z)  # p(z | xy)
    ...
    def count_trigram_events(self, trigram: Trigram) -> None:
        """Count one token of the trigram and also of its suffixes.
        """
        (x, y, z) = trigram
    ...
    def count_trigram_contexts(self, trigram: Trigram) -> None:
        """Count the trigram's context and also the suffixes of that context."""
        (x, y, z) = trigram
    ...
    def train(self, file: Path) -> None:
        ...
        for trigram in get_trigrams(file, self.vocab):
...
class AddLambdaLanguageModel(LanguageModel):
    def __init__(self, vocab: Path, lambda_: float) -> None:
...
class EmbeddingLogLinearLanguageModel(LanguageModel, nn.Module):
    ...
    def __init__(self, vocab: Path, c: float, lexicon: Path) -> None:
        ...
        # TODO: STORE THE LEXICON OF WORD VECTORS IN A USEFUL FORMAT.
        self.dim = None  # TODO: SET THIS TO THE DIMENSIONALITY OF THE VECTORS
        ...
        self.X = nn.Parameter(torch.zeros((self.dim, self.dim)))
        self.Y = nn.Parameter(torch.zeros((self.dim, self.dim)))

    def prob(self, x, y, z) -> float:
        # This returns an ordinary float probability, using the
        # .item() method that extracts a number out of a Tensor.
        return self.log_prob(x, y, z).exp().item()

    def log_prob(self, x: str, y: str, z: str) -> torch.Tensor:
    ...
    def train(self, file: Path):
        ...
        log.info("counting tokens in training corpus...")
        N = num_tokens(file)
        log.info("done.")
        ...
        #####################
        # TODO: Implement your SGD here by taking gradient steps on a
        # sequence of training examples.  Here's how to use PyTorch
        # to make it easy:
        #
        # To get the training examples, you can use the `get_trigrams`
        # function we provided, which will iterate over all N trigrams in the
        # training corpus.
        #
        # For each successive training example i, compute the stochastic
        # objective F_i(θ).  This is called the "forward" computation.
        # Don't forget to include the regularization term.
        #
        # To get the gradient of this objective (∇F_i(θ)), call the
        # `backward` method on the number you computed at the previous
        # step.  This invokes back-propagation to get the gradient of
        # this number with respect to the parameters θ.  This should be
        # easier than implementing the gradient method from the handout.
        #
        ...
        # yourself, or you can call the `step` method of the `optimizer`
        # object we created above.  See the reading handout for more details on this.
        #
        # For the EmbeddingLogLinearLanguageModel, you should run SGD
        # optimization for 10 epochs and then stop.  You might like to show a
        # progress bar using the tqdm library -- just iterate over
        #     tqdm.tqdm(get_trigrams(file), total=10*N)
        # instead of
        #     get_trigrams(file)
        #####################
        ...
        #
        # As Python sees it, your parameters are not actually numbers.  They
        # are `torch.Tensor` objects.  A `Tensor` may represent a numeric
        # scalar, vector, matrix, etc.
        #
        # Every Tensor knows how it was computed.  For example, if you
        # write `a = b + exp(c)`, PyTorch not only computes `a` but also
        # stores backpointers in `a` that remember how the numeric value
        # of `a` depends on the numeric values of `b` and `c`.  In turn,
        # `b` and `c` have their own backpointers that remember what they
        # depend on, and so on, all the way back to the parameters.  This
        # is just like the backpointers in parsing!
        #
        # Every Tensor has a `backward` method that computes the gradient of
        # its numeric value with respect to the parameters, using
        # "back-propagation" through this computation graph.  In particular,
        # once you've computed the forward quantity F_i(θ) as a tensor, you
        # can trace backwards to get its gradient -- i.e., to find out how
        # rapidly it would change if each parameter was slightly changed.
